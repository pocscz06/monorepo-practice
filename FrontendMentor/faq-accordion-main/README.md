...
- Let's say you have some content within a container, and you want to apply the same padding to them. Instead of typing out something like `padding: 1rem;` and `top: 1rem;`, `left: 1rem;` for each, you could create a variable:
    - `--padding: 1rem;` and reuse it as such:
        - `padding: var(--padding);`
        - `top: var(--padding);`
        - `left: var(--padding);`
- When you set something with a position of absolute, ensure that its container has a position set as well, otherwise it'll keep traveling up the parent DOM tree until it finds something to position itself off of (positioning context), even up until it reaches your root HTML element. 
